...
export default function Home() {
  ...
  return (
    <div ref={containerRef} className="flex flex-col min-h-screen bg-slate-50 text-slate-900">
      {/* Hero Section */}
      <section ref={heroRef} className="relative h-[calc(100svh-96px)] md:h-[calc(100svh-100px)] min-h-[600px] w-full overflow-hidden flex flex-col items-center justify-center text-center px-6 bg-slate-50">
        ...
        <div className="absolute inset-0 z-0">
          ...
          <div className="absolute inset-0 bg-white/0 bg-gradient-to-t from-slate-50 via-transparent to-slate-50/60" />
        </div>
        ...
        >
            ...
            <motion.div
              ...
              className="inline-flex items-center space-x-2 bg-white/60 backdrop-blur-md px-4 py-2 border border-slate-200 mb-8 rounded-md"
            >
              <span className="text-slate-900 text-[10px] sm:text-xs font-bold tracking-widest uppercase">
            ...
            <div className="overflow-hidden mb-6 w-full">
              <motion.h1 
                variants={heroReveal}
                className="font-display uppercase tracking-wider text-[4rem] leading-[0.9] sm:text-[6rem] md:text-[8rem] text-slate-900 drop-shadow-sm"
              >
            ...
            >
              <div className="flex items-center text-[10px] sm:text-xs font-bold uppercase tracking-widest text-brand-primary drop-shadow-sm">
              ...
              <div className="flex items-center text-[10px] sm:text-xs font-bold uppercase tracking-widest text-brand-primary drop-shadow-sm">
               ...
               <span className="text-brand-primary/40 text-xs hidden sm:block">•</span>
               <div className="hidden sm:flex items-center text-[10px] sm:text-xs font-bold uppercase tracking-widest text-brand-primary drop-shadow-sm">
            ...
            >
               ...
               <Link 
                href="/shop" 
                className="h-14 sm:h-16 px-10 rounded-md bg-transparent border-2 border-slate-900 text-slate-900 font-bold uppercase tracking-widest text-sm flex items-center justify-center hover:bg-slate-900 hover:text-white transition-colors"
               >
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between md:items-end mb-16">
            <motion.h2 
              ...
              className="font-display uppercase tracking-wide text-5xl md:text-7xl text-slate-900"
            >
              {brand.sections?.featured?.title || "Featured Collections"}
            </motion.h2>
            <motion.p 
              initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp} 
              className="text-slate-600 max-w-sm mt-4 md:mt-0 font-medium"
            >
              ...
              {[...heroCategories, ...heroCategories, ...heroCategories, ...heroCategories].map((collection, idx) => (
                ...
                >
                  <Link 
                    ...
                    className="block w-full h-full overflow-hidden group/card rounded-md bg-slate-100 border border-slate-200 relative"
                  >
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-6 md:p-8 flex flex-col justify-end z-20 transition-transform duration-500">
                      <h3 className="text-white font-display uppercase tracking-wider text-4xl md:text-5xl mb-2 shadow-black drop-shadow-lg group-hover/card:text-brand-primary transition-colors">{collection.name}</h3>
      ...
      {/* Flash Deals Section */}
      {flashDeals.length > 0 && (
        <section className="py-24 bg-slate-50 border-y border-brand-accent/20 relative overflow-hidden">
          {/* Subtle light radial gradient for depth */}
          <div className="absolute inset-x-0 top-0 h-full bg-gradient-to-b from-slate-200 to-transparent pointer-events-none opacity-50 z-0" />
          ...
          <div className="max-w-7xl mx-auto px-6 relative z-10">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
              ...
              >
                ...
                <h2 className="font-display uppercase tracking-wide text-5xl md:text-7xl text-slate-900 drop-shadow-sm">
              ...
              >
                <Link href="/shop?category=deals" className="h-8 px-8 bg-transparent border-2 border-slate-900 text-slate-900 font-bold hover:bg-slate-900 hover:text-white rounded-md transition-colors flex items-center justify-center uppercase tracking-widest text-sm">
              ...
              {flashDeals.map((product) => {
                return (
                 <motion.div 
                  initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp} 
                  key={product.id} 
                  className="relative min-w-[75vw] sm:min-w-[45vw] md:min-w-0 snap-center group flex flex-col bg-white border border-slate-200 hover:border-brand-accent shadow-sm hover:shadow-md transition-all overflow-hidden rounded-md"
                 >
                  <Link href={`/product/${product.id}`} className="block relative aspect-[4/3] bg-slate-100 overflow-hidden group-hover:opacity-90 transition-opacity rounded-t-md">
                    {/* Discount Badge */}
                    ...
                    {/* Social Proof Badge */}
                     <div className="absolute top-2 right-2 z-20 bg-white/90 backdrop-blur-md text-slate-900 border rounded-md border-slate-200 text-[10px] sm:text-xs font-bold px-3 py-1.5 uppercase tracking-widest flex items-center shadow-sm">
                  ...
                  <div className="px-5 pt-4 pb-0 flex flex-col">
                    <Link href={`/product/${product.id}`}>
                      <h3 className="font-poppins font-semibold text-md text-slate-900 group-hover:text-brand-accent transition-colors">
                        {product.name}
                      </h3>
                      <div className="mt-2 flex items-center gap-3">
                        <span className="font-display tracking-widest text-xl text-brand-primary">{formatPrice(product.price)}</span>
                        {product.originalPrice && (
                          <span className="text-slate-400 font-semibold line-through text-sm">{formatPrice(product.originalPrice)}</span>
                        )}
                      </div>
                    </Link>
                  </div>
                  ...
                  <div className="p-4 mt-auto flex flex-col gap-2 relative z-30">
                      ...
                      <Link 
                        ...
                        className="w-full bg-transparent border rounded-md border-slate-300 text-slate-900 font-bold py-2.5 hover:bg-slate-900 hover:text-white transition-colors flex justify-center items-center uppercase tracking-widest text-xs"
                       >
                         Select Option
                       </Link>
                  </div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="flex flex-col md:flex-row justify-between md:items-end mb-16">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp}>
              ...
              <h2 className="font-display uppercase tracking-wide text-5xl md:text-7xl text-slate-900">
                {brand.sections?.newArrivals?.title || "Latest Styles"}
              </h2>
              <p className="text-slate-600 mt-4 max-w-xl font-medium text-base md:text-lg">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp} className="mt-8 md:mt-0">
              <Link href="/shop?category=new-arrivals" className="h-12 px-8 border border-slate-300 text-slate-900 font-bold hover:bg-slate-900 hover:text-white transition-colors flex items-center justify-center uppercase tracking-widest text-sm group rounded-md">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-8">
            {newArrivals.map((product) => (
              ...
              >
                <div className="relative aspect-[3/4] bg-slate-100 border border-slate-200 overflow-hidden mb-5 block rounded-md">
                ...
                <div className="flex-1 flex flex-col text-center px-1">
                  ...
                  <Link href={`/product/${product.id}`}>
                    <h3 className="font-poppins font-semibold text-slate-900 text-base sm:text-lg line-clamp-1 mb-2 group-hover:text-brand-primary transition-colors">
                      {product.name}
                    </h3>
                  </Link>
                  <div className="flex items-center justify-center gap-3">
                    <span className="font-display tracking-widest text-xl text-slate-900">{formatPrice(product.price)}</span>
                  </div>
                  ...
                  <div className="mt-4 flex flex-col gap-2 w-full">
                     ...
                     <Link 
                      href={`/product/${product.id}`}
                      className="w-full bg-transparent border border-slate-300 text-slate-900 font-bold py-2 rounded-md transition-colors flex justify-center items-center uppercase tracking-widest text-[10px] sm:text-xs hover:bg-slate-900 hover:text-white"
                     >
                       Select Option
                     </Link>
                  </div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="flex flex-col md:flex-row justify-between md:items-end mb-16">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp} className="mt-6 md:mt-0">
              <Link href="/shop?category=best-sellers" className="h-12 px-8 border border-slate-300 text-slate-900 font-bold hover:bg-slate-900 hover:text-white transition-colors flex items-center justify-center uppercase tracking-widest text-sm group rounded-md">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-8">
            {bestSellers.slice(0, 4).map((product) => (
              ...
              >
                <div className="relative aspect-[4/5] bg-slate-100 border border-slate-200 overflow-hidden mb-5 block group-hover:border-brand-accent transition-colors rounded-md">                  
                  <Link href={`/product/${product.id}`} className="block w-full h-full absolute inset-0 z-10">
                    <Image
                      src={product.image}
                      alt={product.name}
                ...
                <div className="flex-1 flex flex-col text-left px-1">
                  <div className="flex items-center gap-1 mb-2">
                    ...
                    <span className="text-slate-500 text-xs font-bold ml-1">(120+ Reviews)</span>
                  </div>
                  <Link href={`/product/${product.id}`}>
                    <h3 className="font-poppins font-semibold text-slate-900 text-base sm:text-lg line-clamp-1 mb-1 group-hover:text-brand-accent transition-colors">
                      {product.name}
                    </h3>
                  </Link>
                  <div className="flex items-center gap-2 mb-3">
                    <span className="font-display tracking-widest text-sm text-slate-900">{formatPrice(product.price)}</span>
                    {product.originalPrice && (
                       <span className="text-slate-400 font-semibold line-through text-xs sm:text-sm">{formatPrice(product.originalPrice)}</span>
                    )}
                  </div>
                  
                  <div className="text-[9px] sm:text-[10px] text-slate-500 uppercase tracking-widest mb-4 flex items-center">
                    <ShoppingBag className="h-3 w-3 mr-1 text-slate-400" /> {brand.salesCallout}
                  </div>
      ...
      <section className="py-24 bg-white border-t border-slate-200 relative">
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="text-center mb-16">
            ...
            <motion.h2 initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp} className="font-display uppercase tracking-wide text-4xl md:text-6xl text-slate-900 mb-6">
              {brand.sections?.whyUs?.titleTop || "WHY SHOP WITH"}<br/>
              {brand.name.toUpperCase()}
            </motion.h2>
            <motion.p initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp} className="text-slate-600 max-w-2xl mx-auto font-medium text-base md:text-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {/* Feature 1 */}
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp} className="bg-slate-50 min-h-[220px] p-8 md:p-10 border border-slate-200 hover:border-brand-primary/50 transition-all duration-300 group cursor-default rounded-md">
              <div className="bg-slate-200 w-14 h-14 flex items-center justify-center mb-6 group-hover:bg-brand-primary/10 transition-colors rounded-md">
                <Star className="h-6 w-6 text-slate-800 group-hover:text-brand-primary transition-colors" />
              </div>
              <h3 className="font-display tracking-widest uppercase text-xl text-slate-900 mb-3">{brand.features[0].title}</h3>
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp} className="bg-slate-50 min-h-[220px] p-8 md:p-10 border border-slate-200 hover:border-brand-primary/50 transition-all duration-300 group cursor-default rounded-md">
              <div className="bg-slate-200 w-14 h-14 flex items-center justify-center mb-6 group-hover:bg-brand-primary/10 transition-colors rounded-md">
                <Wallet className="h-6 w-6 text-slate-800 group-hover:text-brand-primary transition-colors" />
              </div>
              <h3 className="font-display tracking-widest uppercase text-xl text-slate-900 mb-3">{brand.features[1].title}</h3>
              <p className="text-slate-600 text-sm leading-relaxed">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp} className="bg-slate-50 min-h-[220px] p-8 md:p-10 border border-slate-200 hover:border-brand-primary/50 transition-all duration-300 group cursor-default rounded-md">
              <div className="bg-slate-200 w-14 h-14 flex items-center justify-center mb-6 group-hover:bg-brand-primary/10 transition-colors rounded-md">
                <Truck className="h-6 w-6 text-slate-800 group-hover:text-brand-primary transition-colors" />
              </div>
              <h3 className="font-display tracking-widest uppercase text-xl text-slate-900 mb-3">{brand.features[2].title}</h3>
              <p className="text-slate-600 text-sm leading-relaxed">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp} className="bg-slate-50 min-h-[220px] p-8 md:p-10 border border-slate-200 hover:border-brand-primary/50 transition-all duration-300 group cursor-default rounded-md">
              <div className="bg-slate-200 w-14 h-14 flex items-center justify-center mb-6 group-hover:bg-brand-primary/10 transition-colors rounded-md">
                <MessageCircle className="h-6 w-6 text-slate-800 group-hover:text-brand-primary transition-colors" />
              </div>
              <h3 className="font-display tracking-widest uppercase text-xl text-slate-900 mb-3">{brand.features[3].title}</h3>
              <p className="text-slate-600 text-sm leading-relaxed">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="flex flex-col md:flex-row justify-between md:items-end mb-16">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp}>
              ...
              <h2 className="font-display uppercase tracking-wide text-4xl md:text-6xl text-slate-900 mb-6">
              ...
              <div className="flex items-center gap-4 mt-6">
                <div className="flex -space-x-3">
                  {reviewAvatars.map((src, idx) => (
                    <Image key={idx} src={src} width={40} height={40} className="w-10 h-10 rounded-full border-2 border-slate-50 object-cover" alt={`User ${idx + 1}`} />
                  ))}
                </div>
                <div>
                  ...
                  <p className="text-slate-900 text-[10px] sm:text-xs font-bold tracking-widest uppercase">{reviewStats.averageRating} • {reviewStats.totalCustomers}</p>
                </div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="overflow-hidden pb-8 md:pb-0 -mx-6 px-6 md:mx-0 md:px-0 relative group">
            ...
            >
              {[...testimonials, ...testimonials, ...testimonials, ...testimonials].map((review, idx) => (
                <div 
                  key={`${review.id}-${idx}`} 
                  className="w-[85vw] sm:w-[350px] md:w-[400px] shrink-0 bg-white border border-slate-200 hover:border-brand-primary/30 p-6 flex flex-col group/review rounded-md transition-all duration-500 hover:-translate-y-2 shadow-sm hover:shadow-md"
                >
                  ...
                  <p className="text-slate-700 text-base md:text-lg italic mb-8 flex-1 leading-relaxed">
                  ...
                  <div className="flex items-center gap-4 mt-auto">
                    <Image
                      ...
                      width={48}
                      height={48}
                      className="rounded-full object-cover border-2 border-slate-200 group-hover/review:border-brand-primary/50 transition-colors"
                    />
                    <div>
                      <p className="text-slate-900 font-bold text-sm tracking-wide">{review.name}</p>
                      <p className="text-slate-500 text-xs mt-1 flex items-center font-medium">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="flex flex-col items-start text-left">
              ...
              <motion.h2 initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp} className="font-display uppercase tracking-wide text-5xl md:text-7xl text-slate-900 mb-6 leading-[1.1]">
              ...
              <motion.p initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={fadeUp} className="text-slate-700 font-medium text-lg md:text-xl mb-10 max-w-lg leading-relaxed">
            ...
            >
              <div className="absolute inset-x-0 bottom-0 top-1/2 bg-gradient-to-t from-white to-transparent z-10 pointer-events-none"></div>
              
              <div className="bg-slate-50 border border-slate-300 rounded-xl overflow-hidden shadow-2xl relative">
                {/* Chat Header */}
                <div className="bg-white px-6 py-4 flex items-center border-b border-slate-200">
                   {/* DYNAMIC LOGO / FALLBACK SHORTNAME */}
                   <div className="relative w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center mr-4 border border-slate-200 overflow-hidden shrink-0">
                     {brand.logo ? (
                   ...
                   <div>
                     <p className="text-slate-900 font-bold text-sm">{brand.name}</p>
                ...
                {/* Chat Body */}
                <div className="p-6 pb-20 space-y-4 bg-slate-100/50">
                   
                   {brand.whatsappMockChat.map((msg, idx) => (
                      msg.sender === 'brand' ? (
                        <div key={idx} className="flex w-full mt-2 space-x-3 max-w-xs">
                          {/* DYNAMIC LOGO / FALLBACK SHORTNAME */}
                          <div className="relative flex-shrink-0 w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center mt-auto border border-slate-300 overflow-hidden">
                             {brand.logo ? (
                               <Image src={brand.logo} alt={brand.name} fill className="object-cover" />
                             ) : (
                               <span className="font-display text-slate-900 text-xs">
                                 {brand.shortName || brand.name.substring(0, 2).toUpperCase()}
                               </span>
                             )}
                          </div>
                          <div className="bg-white p-4 rounded-xl rounded-bl-sm border border-slate-200 shadow-md">
                        ...
                        <div key={idx} className="flex w-full mt-2 space-x-3 max-w-sm ml-auto justify-end">
                          <div className="bg-brand-primary/20 p-4 rounded-xl rounded-br-sm border border-brand-primary/30 shadow-md">
                             ...
                             <div className="flex justify-end items-center mt-1 space-x-1">
                                <p className="text-slate-600 text-[10px]">{msg.time}</p>
                ...
                {/* Chat Input */}
                <div className="absolute bottom-0 inset-x-0 bg-white p-4 flex items-center border-t border-slate-200 z-20">
                   <div className="bg-slate-50 w-full rounded-md h-10 flex items-center px-4 border border-slate-200">
                      <p className="text-slate-400 text-sm">Message...</p>
                   </div>
                   <div className="w-10 h-10 rounded-md bg-brand-primary flex items-center justify-center ml-3 flex-shrink-0">
                      <MessageCircle className="w-5 h-5 text-black fill-slate-900" />
                   </div>
                </div>
              </div>
            </motion.div>
